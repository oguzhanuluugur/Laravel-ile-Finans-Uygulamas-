            ...
            <div class="col-md-12 widget-holder">
                <div class="widget-bg">
                    <div class="widget-body clearfix">
                        ...
                        <form action="{{route('islem.store',['type'=>0])}}" enctype="multipart/form-data" method="POST" >
                            @csrf
                            <div class="form-group row ">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="faturaId"  class="m-b-10 form-control fatura" data-placeholder="Choose" data-toggle="select2">
                                            @foreach(\App\Models\Fatura::getList(FATURA_GIDER) as $key=>$value)
                                                <option data-fiyat="{{\App\Models\Fatura::getTotal($value['id'])}}" data-musteriId="{{$value['musteriId']}}" value="{{$value['id']}}">{{$value['faturaId']}} / {{\App\Models\Müsteriler::getPublicName($value['musteriId'])}} / {{\App\Models\Fatura::getTotal($value['id'])}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="date" class="form-control" required name="tarih" value="{{ date('Y-m-d') }}" >
                                </div>
